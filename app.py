...
def tab2_content():
    ...
    if rides is not None:
        ...
        st.write('Your ride details and analytics will be show up here once you log them.')
...
def tab3_content():
    ...
    with col1:
        with st.expander('Create a Ride', expanded = True):
            ...
            create_ride = st.button('Create a new ride', use_container_width=True)
            if create_ride:
                ride_date = json.dumps(ride_date.isoformat())
                ride_details = {
                    'Created_By' : creator,
                    'Ride_ Name' : ride_name,
                    'Starting_Location' : starting_loc,
                    'Destination_Location' : destination_loc,
                    'Ride_Date' : ride_date,
    ...
    with col3:
        with st.expander('Join A Ride', expanded = True):
            st.write('Join us mate!')
            ride_code = st.text_input('Enter the Ride Code')
            join_btn = st.button('Join A Ride', use_container_width=True)
            if join_btn:
                if ride_code == "":
                    st.warning('Please enter the Ride Code.')
                else:
                    join_ride = db.child("--Created Rides--").child(ride_code).get().val()

                    user = st.session_state['user_id']
                    db.child(user['localId']).child("Rides").child("Joined Rides").child(ride_code).set(join_ride)
                    st.toast('You have succesfully joined the Ride.', icon='✅')
...
def tab4_content():
    ...
    with st.expander('Upload Documents'):
        doc_list = ('--Select--' ,'Driving License', 'Registration Certificate', 'Insurance Certificate', 'PUC Certificate', 'Aadhaar Card')
        doc_name = st.selectbox("Select Document", doc_list) 
        uploaded_file = st.file_uploader("Upload the document")
        upload_btn  = st.button('Upload File')

        if upload_btn:
            if doc_name != "--Select--":
                if uploaded_file is not None:
                    file_storage = storage.child(f"{username}/{doc_name}")
                    file_storage.put(uploaded_file)

                    url = storage.child(f"{username}/{doc_name}").get_url(token=None)

                    # Store URL of the document in RealTime Database
                    db.child(user['localId']).child("User Documents").child(doc_name).set(url)

                    st.toast('Your document has been successfully uploaded. ✅')
                else:
                    st.warning('Please upload the document.')
            else:
                st.warning('Please select the document.')

    with st.expander('View Documents'):
        uploaded_docs = db.child(user['localId']).child("User Documents").get().val()
        if uploaded_docs is not None:
            uploaded_docs = uploaded_docs.keys()
            for doc_name in uploaded_docs:
                if doc_name != '--Select--':
                    url = storage.child(f"{username}/{doc_name}").get_url(token=None)
                    if url:
                        st.link_button(doc_name, url)
        else:
            st.write('you have not uploaded any documents to view them.')
...
def runapp():
    ...
    if st.session_state['user_status'] == "logged_in": # which means user is logged in
        banner = Image.open('assets/BikeBuddy Banner.png')
        st.image(banner)
